...
export default function CastTile(props:ICastTileProps) {
    ...
    function showImage() {
        if (props.castMember === undefined) {
            return (
                <div className="placeholderGradientAnimation" style={{position:'absolute', backgroundColor:props.hue.defaults.textLarge, borderRadius:'10px 0px 0px 10px', top:0, bottom:0, display:'flex', width:66,alignItems:'center', justifyContent:'center'}}/>
            )
        }
    ...
    return (
        <div style={{display:'flex', backgroundColor:props.hue.defaults.panel, boxShadow:`inset 0px 0px 0px 1px ${props.hue.defaults.border}`, borderRadius:10}}>
            ...
            <div style={{padding:10, color:props.hue.defaults.textSmall}}>
                {props.castMember === undefined ? <p className="placeholderGradientAnimation" style={{backgroundColor:props.hue.defaults.textLarge, color:'transparent', borderRadius:8, width:'fit-content'}}>writer names</p> : <p style={{fontWeight:600}}>{props.castMember.name}</p>}
                {/* <p style={{fontWeight:600}}>{props.castMember.name}</p> */}
                {props.castMember === undefined ? <p className="placeholderGradientAnimation" style={{backgroundColor:props.hue.defaults.textLarge, color:'transparent', borderRadius:8, width:'fit-content', marginTop:5}}>writer names go here</p> : <p>as {props.castMember.character}</p>}
